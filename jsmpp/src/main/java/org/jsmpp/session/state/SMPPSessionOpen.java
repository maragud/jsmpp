...
class SMPPSessionOpen implements SMPPSessionState {
        ...
        if (commandCompletableFuture != null) {
            try {
                ...
                responseHandler.sendGenerickNack(e.getErrorCode(), pduHeader
                        .getSequenceNumber());
                commandCompletableFuture
                        .completeExceptionally(new InvalidResponseException(
                                message, e));
            }
        } else {
    ...
    public void processReplaceSmResp(Command pduHeader, byte[] pdu,
            ResponseHandler responseHandler) {
        CompletableFuture<Command> commandCompletableFuture = responseHandler.removeSentItemAsync(1);
